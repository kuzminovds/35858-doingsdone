...
if (!empty($_POST)) {
	...
	$checkEmail = email_used($email);
	if ($checkEmail != [] AND $checkEmail[0][0] == $email) {
	...
	if ($login_errors == []) {
		...
		$add_user = $db->insertData($sql, [$name, $email, $password]);
					...
					<form class="form" class="" action="register.php" method="post">
						<div class="form__row">
							...
							<?php if (isset($login_errors['email']) OR isset($login_errors['email-have'])): ?>
								<input class="form__input form__input--error" type="text" name="email" id="email" value="<?=$_POST["email"]; ?>" placeholder="Введите e-mail">
